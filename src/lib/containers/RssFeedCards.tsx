...
export default class RssFeed extends React.Component<RssFeedProps, RssState> {
  ...
  componentDidMount() {
    ...
    let wp:wpapi = new wpapi(options)
    wp.posts().then((result:any) => {
      debugger
      this.setState({ rssFeed: result.content })
    }).catch(err => {
      if (this._isMounted) {
      ...
      <>
        ...
        <div className="RssFeed">
          ...
          {this.state.isLoadingError && (
            <div>Unable to load feed: {this.props.options.endpoint}</div>
          )}
        </div>
      </>
    )
